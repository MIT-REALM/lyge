...
class PDQuadraticNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.quadratic(x) + 0.5 * (self.f(x) ** 2).sum(dim=1, keepdim=True)
    ...
    def forward_jacobian(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        y = y_f + self.quadratic(x)
        jacobian = jacobian_f + 2 * torch.matmul(x, self.quadratic_matrix()).unsqueeze(1)
